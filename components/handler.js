...
class Handler {
  ...
  getFabric({ directory, minecraftVersion, loaderVersion }) {
    const API_URL = `https://meta.fabricmc.net/v2/versions/loader/${minecraftVersion}/${loaderVersion}/profile/json`;
    const fabricLoader = path.join(
      directory,
      `fabric-loader-${loaderVersion}-${minecraftVersion}.json`,
    );
    return new Promise((resolve) => {
      request.get(API_URL, (error, response, body) => {
        if (error) {
          this.client.emit(
            'debug',
            `[CaveSpider Core]: Failed to download Fabric loader due to ${error}`,
          );
        ...
        try {
          ...
          this.client.emit(
            'debug',
            '[CaveSpider Core]: Downloaded Fabric loader',
          );
          resolve(true);
        } catch (parseError) {
          this.client.emit(
            'debug',
            `[CaveSpider Core]: Failed to parse Fabric loader JSON due to ${parseError}`,
          );
          resolve(false);
        }
      });
    ...
    return new Promise((resolve) => {
      child.exec(`"${java}" -version`, (error, stdout, stderr) => {
        if (error) {
          ...
          this.client.emit(
            'debug',
            `[CaveSpider Core]: Using Java version ${stderr.match(/"(.*?)"/).pop()} ${stderr.includes('64-Bit') ? '64-bit' : '32-Bit'}`,
          );
          resolve({
            run: true,
          });
    ...
    return new Promise((resolve) => {
      ...
      _request.on('response', (data) => {
        if (data.statusCode === 404) {
          this.client.emit(
            'debug',
            `[CaveSpider Core]: Failed to download ${url} due to: File not found...`,
          );
          return resolve(false);
        }
      ...
      _request.on('error', async (error) => {
        this.client.emit(
          'debug',
          `[CaveSpider Core]: Failed to download asset to ${path.join(directory, name)} due to\n${error}.` +
            ` Retrying... ${retry}`,
        );
    ...
    return new Promise((resolve, reject) => {
      checksum.file(file, (err, sum) => {
        if (err) {
          this.client.emit(
            'debug',
            `[CaveSpider Core]: Failed to check file hash due to ${err}`,
          );
          return resolve(false);
        }
      ...
      request.get(manifest, (error, response, body) => {
        ...
        if (!error) {
          if (!fs.existsSync(cache)) {
            ...
            this.client.emit(
              'debug',
              '[CaveSpider Core]: Cache directory created.',
            );
          }
          fs.writeFile(
            ...
            (err) => {
              ...
              this.client.emit(
                'debug',
                '[CaveSpider Core]: Cached version_manifest.json',
              );
            },
          );
        }
        const parsed =
          error && error.code === 'ENOTFOUND'
            ...
            : JSON.parse(body);
        const desiredVersion = Object.values(parsed.versions).find(
        ...
        if (desiredVersion) {
          request.get(desiredVersion.url, (error, response, body) => {
            ...
            if (!error) {
              fs.writeFile(
                ...
                (err) => {
                  ...
                  this.client.emit(
                    'debug',
                    `[CaveSpider Core]: Cached ${this.options.version.number}.json`,
                  );
                },
              );
            }
            ...
            this.client.emit(
              'debug',
              '[CaveSpider Core]: Parsed version from version manifest',
            );
            this.version =
              error && error.code === 'ENOTFOUND'
                ? JSON.parse(
  ...
  async getJar() {
    ...
    return this.client.emit(
      'debug',
      '[CaveSpider Core]: Downloaded version jar and wrote version json',
    );
  }

  async getAssets() {
    const assetDirectory = path.resolve(
      this.options.overrides.assetRoot ||
        path.join(this.options.root, 'assets'),
    );
    ...
    if (this.isLegacy()) {
      if (fs.existsSync(path.join(assetDirectory, 'legacy'))) {
        this.client.emit(
          'debug',
          "[CaveSpider Core]: The 'legacy' directory is no longer used as Minecraft looks " +
      ...
      this.client.emit(
        'debug',
        `[CaveSpider Core]: Copying assets over to ${legacyDirectory}`,
      );
  ...
  async getNatives() {
    ...
    if (
      ...
      this.client.emit(
        'debug',
        '[CaveSpider Core]: Downloaded and extracted natives',
      );
    }
    ...
    this.client.emit(
      'debug',
      `[CaveSpider Core]: Set native path to ${nativeDirectory}`,
    );
  ...
  async getForgedWrapped() {
    ...
    if (fs.existsSync(versionPath)) {
      try {
        ...
        if (
          ...
          this.client.emit(
            'debug',
            '[CaveSpider Core]: Old ForgeWrapper has generated this version JSON, re-generating',
          );
        } else {
        ...
        this.client.emit(
          'debug',
          '[CaveSpider Core]: Failed to parse Forge version JSON, re-generating',
        );
      }
    }
    ...
    this.client.emit(
      'debug',
      '[CaveSpider Core]: Generating Forge version json, this might take a bit',
    );
    ...
    try {
      ...
      this.client.emit(
        'debug',
        '[CaveSpider Core]: Failed to load json files for ForgeWrapper, using Vanilla instead',
      );
      return null;
    }
    if (installerJson) {
        ...
        for (const library of json.mavenFiles) {
          ...
          if (lib[0] === 'net.minecraftforge' && lib[1].includes('forge')) {
            library.downloads.artifact.url =
              this.options.overrides.url.mavenForge +
        ...
        json.libraries.map(async (library) => {
          ...
          this.baseRequest(downloadLink, (error, response, body) => {
            if (error) {
              this.client.emit(
                'debug',
                `[CaveSpider Core]: Failed checking request for ${downloadLink}`,
              );
            } else {
  ...
  formatQuickPlay() {
    ...
    if (!keys.includes(type)) {
      this.client.emit(
        'debug',
        `[CaveSpider Core]: quickPlay type is not valid. Valid types are: ${keys.join(', ')}`,
      );
      return null;
    }
    const returnArgs =
  ...
  async getLaunchOptions(modification) {
    ...
    const fields = {
      ...
      '${version_name}':
        this.options.version.number || this.options.overrides.versionName,
      '${assets_index_name}':
    ...
    if (this.options.server)
      this.client.emit(
        'debug',
        '[CaveSpider Core]: server and port are deprecated launch flags. Use the quickPlay field.',
      );
  ...
  getMemory() {
    if (!this.options.memory) {
      this.client.emit(
        'debug',
        '[CaveSpider Core]: Memory not set! Setting 1GB as MAX!',
      );
      this.options.memory = {
        min: 512,
        max: 1023,
    ...
    if (!isNaN(this.options.memory.max) && !isNaN(this.options.memory.min)) {
      if (this.options.memory.max < this.options.memory.min) {
        this.client.emit(
          'debug',
          '[CaveSpider Core]: MIN memory is higher then MAX! Resetting!',
        );

...
class Handler {
  ...
  async getForgedWrapped () {
    ...
    // Since we're building a proper "custom" JSON that will work nativly with CaveSpider Core, the version JSON will not
    ...
    if (fs.existsSync(versionPath)) {
      try {
        ...
        if (!json.forgeWrapperVersion || !(json.forgeWrapperVersion === this.options.overrides.fw.version)) {
          ...
          // If forge is modern, add ForgeWrappers launch arguments and set forge to null so CaveSpider Core treats it as a custom json.
    ...
    // Adding the installer libraries as mavenFiles so CaveSpider Core downloads them but doesn't add them to the class paths.
    if (installerJson) {
      json.mavenFiles
    ...
    if (this.isModernForge(json)) {
      // If forge is modern and above 1.12.2, we add ForgeWrapper to the libraries so CaveSpider Core includes it in the classpaths.
      ...
      // Modifying legacy library format to play nice with CaveSpider Core's downloadToDirectory function.
    ...
    // Make CaveSpider Core treat modern forge as a custom version json rather then legacy forge.

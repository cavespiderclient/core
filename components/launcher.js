...
class CaveSpiderCore extends EventEmitter {
  async launch(options) {
    try {
      ...
      if (!java.run) {
        this.emit(
          'debug',
          `[CaveSpider Core]: Couldn't start Minecraft due to: ${java.message}`,
        );
        this.emit('close', 1);
        return null;
      }
      ...
      const fabricDirectory = path.join(
        this.options.root,
        'versions',
        `fabric-loader-${this.options.fabric.version}-${this.options.version.number}`,
      );
      const fabricLoader = path.join(
        fabricDirectory,
        `fabric-loader-${this.options.fabric.version}-${this.options.version.number}.json`,
      );
      ...
      if (this.options.fabric) {
        ...
        if (!fs.existsSync(fabricDirectory) || !fs.existsSync(fabricLoader)) {
          this.emit(
            'debug',
            '[CaveSpider Core]: Attempting to download Fabric loader',
          );
      ...
      if (!fs.existsSync(mcPath)) {
        this.emit(
          'debug',
          '[CaveSpider Core]: Attempting to download Minecraft version jar',
        );
      ...
      if (this.options.overrides.logj4ConfigurationFile) {
        jvm.push(
          `-Dlog4j.configurationFile=${path.resolve(this.options.overrides.logj4ConfigurationFile)}`,
        );
      }
      // https://help.minecraft.net/hc/en-us/articles/4416199399693-Security-Vulnerability-in-Minecraft-Java-Edition
      if (
        parseInt(versionFile.id.split('.')[1]) === 18 &&
        !parseInt(versionFile.id.split('.')[2])
      )
      ...
      this.emit(
        'debug',
        `[CaveSpider Core]: Using ${separator} to separate class paths`,
      );
      ...
      this.emit(
        'debug',
        `[CaveSpider Core]: Launching with arguments ${launchArguments.join(' ')}`,
      );
      ...
      this.emit(
        'debug',
        `[CaveSpider Core]: Failed to start due to ${e}, closing...`,
      );
      return null;
    }
  }
  ...
  printVersion() {
    if (fs.existsSync(path.join(__dirname, '..', 'package.json'))) {
      ...
      this.emit(
        'debug',
        `[CaveSpider Core]: CaveSpider Core version ${version}`,
      );
    } else {
      this.emit(
        'debug',
        '[CaveSpider Core]: Package JSON not found, skipping CaveSpider Core version check.',
      );
    }
  }
  ...
  async extractPackage() {
    if (this.options.clientPackage) {
      this.emit(
        'debug',
        `[CaveSpider Core]: Extracting client package to ${this.options.root}`,
      );
  ...
  async getModifyJson() {
    ...
    if (this.options.forge) {
      ...
      this.emit(
        'debug',
        '[CaveSpider Core]: Detected Forge in options, getting dependencies',
      );
      modifyJson = await this.handler.getForgedWrapped();
    } else if (this.options.version.custom) {
      this.emit(
        'debug',
        '[CaveSpider Core]: Detected custom in options, setting custom version file',
      );
      modifyJson =
        modifyJson ||
        JSON.parse(
